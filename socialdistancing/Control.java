...
public class Control {
		...
		//Model and View
		ArrayList<Citizen> model; //the community of citizen objects	
		...
		//position extents, keep objects away from to edges
		...
		public Control(int np, double roam, double infected, double die, int sicklow, int sickMax) {
			...
			for(int i = 0; i < Control.numPeople; i++) {
				//instantiate Citizen object and add it to the ArrayList
		...
		public void controlPaint(Graphics g) {
			...
			for(Citizen p: model) {
				...
				p.healthManager();  //manage health values of each Citizen/Person
				p.velocityManager();  //manage position values of Citizen/Person
